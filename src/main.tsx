...
function makeBoard(name='Sprint Retrospective', template=templates[0]): Board {
  return { id: uid(), name, revealed: true, createdAt: Date.now(), columns: template.columns.map(([title,emoji]) => ({ id: uid(), title, emoji, cards: [] })) };
}

function App() {
  const [board, setBoard] = useState<Board>(() => {
    const hash = location.hash.startsWith('#b=') ? location.hash.slice(3) : '';
    if (hash) { try { return JSON.parse(decodeURIComponent(atob(hash))); } catch {} }
    const saved = localStorage.getItem('boardly-board');
    return saved ? JSON.parse(saved) : makeBoard();
  });
  const [name, setName] = useState(localStorage.getItem('boardly-name') || 'Anonymous');
  ...
  const totalVotes = useMemo(() => board.columns.reduce((n,c)=>n+c.cards.reduce((m,x)=>m+x.votes,0),0), [board]);

  function flash(msg:string){ setToast(msg); setTimeout(()=>setToast(''),1800); }
  function addCard(columnId:string){
    const text=(drafts[columnId]||'').trim(); if(!text) return;
    setBoard(b=>({...b,columns:b.columns.map(c=>c.id===columnId?{...c,cards:[...c.cards,{id:uid(),text,votes:0,author:name||'Anonymous'}]}:c)}));
    setDrafts(d=>({...d,[columnId]:''}));
  }
  function vote(columnId:string, cardId:string){
    setBoard(b=>({...b,columns:b.columns.map(c=>c.id===columnId?{...c,cards:c.cards.map(x=>x.id===cardId?{...x,votes:x.votes+1}:x)}:c)}));
  }
  function removeCard(columnId:string, cardId:string){
    setBoard(b=>({...b,columns:b.columns.map(c=>c.id===columnId?{...c,cards:c.cards.filter(x=>x.id!==cardId)}:c)}));
  }
  function applyTemplate(t:any){ setBoard(makeBoard(board.name,t)); setShowTemplates(false); }
  function resetBoard(){ if(confirm('Clear all cards on this board?')) setBoard(b=>({...b,columns:b.columns.map(c=>({...c,cards:[]}))})); }
  async function share(){
    const url=location.href;
    try { await navigator.clipboard.writeText(url); flash('Board link copied'); } catch { flash('Copy the URL from your browser'); }
  }
  ...
  return <div className="app-shell">
    ...
    <header className="topbar">
      ...
      <div className="top-actions">
        ...
        <button className="ghost" onClick={()=>setBoard(b=>({...b,revealed:!b.revealed}))}>{board.revealed?<EyeOff size={17}/>:<Eye size={17}/>} {board.revealed?'Hide':'Reveal'}</button>
    ...
    <main>
      ...
      {showTemplates && <div className="template-strip">
        {templates.map(t=><button key={t.name} onClick={()=>applyTemplate(t)}><strong>{t.name}</strong><span>{t.columns.map((x:any)=>x[1]).join(' ')}</span></button>)}
      </div>}
      ...
      <section className="columns" style={{gridTemplateColumns:`repeat(${Math.min(board.columns.length,5)}, minmax(260px,1fr))`}}>
        {board.columns.map((col,idx)=><div className="column" key={col.id}>
          ...
          <div className="composer">
            <textarea value={drafts[col.id]||''} onChange={e=>setDrafts({...drafts,[col.id]:e.target.value})} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();addCard(col.id)}}} placeholder={`Add a thought to “${col.title}”...`}/>
            <button onClick={()=>addCard(col.id)}><Plus size={16}/> Add card</button>
          </div>
          <div className="cards">
            {!board.revealed && col.cards.length>0 ? <div className="hidden-stack"><EyeOff size={20}/><strong>{col.cards.length} hidden {col.cards.length===1?'card':'cards'}</strong><span>Reveal when everyone is ready.</span></div> : col.cards.map(card=><article className="card" key={card.id}>
              <p>{card.text}</p>
              <div className="card-foot"><span>{card.author}</span><div><button onClick={()=>vote(col.id,card.id)} className="vote"><ThumbsUp size={14}/>{card.votes||''}</button><button onClick={()=>removeCard(col.id,card.id)} className="trash"><Trash2 size={14}/></button></div></div>
            </article>)}
      ...
      <section className="bottom-callout">
        ...
        <div><strong>Retro flow built in</strong><span>Write privately → reveal together → vote → discuss → turn decisions into actions.</span></div>
        <button onClick={share}><Copy size={16}/> Copy board link</button>
      </section>
    </main>
